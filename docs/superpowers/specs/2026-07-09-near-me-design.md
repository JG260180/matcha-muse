...
3. **Filters:**
   ...
   - **Milk:** checkbox chips `dairy, oat, soy, almond, coconut, other`, plus a seventh chip **"Unspecified"** for reviews where no milk was recorded — **all pre-ticked by default**. Unticking a box excludes that milk from the results. (Exclusion model: the user removes what she doesn't want, e.g. untick dairy to see only alternative milks.) `other` means obscure milks deliberately recorded as "other"; it is NOT a bucket for missing data — missing milk is its own `Unspecified` bucket. All boxes unticked = empty result set with the standard empty-state message.
4. **List, grouped by cafe as card stacks** — the list's first job is location, then ratings, so the unit of the list is the **cafe**, not the individual matcha. One entry per cafe matching the filters:
   ...
   - Sorting is at cafe level: `Nearest` by distance; `Top rated` by that cafe's **best** overall score among matching reviews (a cafe with one 5★ matcha outranks one with three 4★s). The collapsed stack still *displays* the average, but ordering uses the best.
...
- **Unit (TDD):**
  ...
  - Filter/group/sort pure function over a fixture set of reviews: serve filter; milk exclusion filter (all ticked = all shown, unticked milk excluded, null milk in its own `unspecified` bucket distinct from `other`); grouping by cafe with per-cafe count, average (display) and best (sort) of matching reviews; Nearest sort (missing-coordinate cafe sorts last); Top-rated sort by cafe best score.
