...
3. **Filters:**
   ...
   - **Milk:** multi-select chips `dairy, oat, soy, almond, coconut, other`. None selected = show all milks. Any selected = show only matchas whose milk is in the selected set. This is the "alternative milks" filter (e.g. select oat + almond).
4. **List of matchas** — one card per review matching the filters (individual matchas, not grouped by cafe). Each card shows: photo (via existing `SignedImage`), cafe name, **distance from user** (e.g. "450 m" / "1.2 km"), overall score, and milk · serve. Sorted per the sort control.
   - Card actions (only for cafes with a `google_place_id`):
     ...
     - **Review on Google** — copies the review `note` to the clipboard (if present), then opens `https://search.google.com/local/writereview?placeid=<place_id>` where the user completes and submits the review herself. (Google provides no API to auto-post consumer reviews; this is the frictionless manual path.)
...
- Grouping the list by cafe (current design lists individual matchas so the per-matcha milk/serve filters read naturally).
...
- **Unit (TDD):**
  ...
  - Filter/sort pure function over a fixture set of reviews: serve filter, milk multi-select filter, Nearest sort (with a missing-coordinate cafe sorting last), Top-rated sort.
  - Google URL builders (`writereview`, `maps directions`, static map `markers`) produce expected strings.
- **Manual (on-device with owner):** location permission grant/deny paths, map renders with pins, filters/sort behave, distances look right, both Google buttons open the correct place, clipboard copy works on iOS Safari.
